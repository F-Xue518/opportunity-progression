...
def build_phase2(doc, p2):
    ...
    add_snapshot_row(doc, [
        {"label": "Deal Health", "value": p2.get("health", ""),
         "color": MD3.get(p2.get("health_color", "primary"), MD3["primary"]), "big": True},
    ...
    if p2.get("immediate_action") or p2.get("targets"):
        ...
        h = c1.paragraphs[0].add_run("🚀  IMMEDIATE ACTION · THIS WEEK")
        h.bold = True
        from docx.shared import Pt
        h.font.size = Pt(9)
        ...
        h = c2.paragraphs[0].add_run("🎯  30-DAY TARGETS")
        h.bold = True
        h.font.size = Pt(9)
        _run_color(h, MD3["primary"])
...
def build_phase25(doc, p25):
    if not p25:
        return
    add_section_title(doc, "Phase 2.5 · Stage Alignment")

    add_snapshot_row(doc, [
        {"label": "Current Stage (reported)", "value": p25.get("current_stage", ""),
         "color": MD3["primary"], "big": True},
        {"label": "Criteria-Supported Stage", "value": p25.get("supported_stage", ""),
         "color": MD3["warning"], "big": True},
        {"label": "Assessment Result", "value": p25.get("result", ""),
         "color": MD3.get(p25.get("result_color", "warning"), MD3["warning"]), "big": True},
    ])

    if p25.get("rationale"):
        cell = add_card(doc, bg_hex=MD3["surface_container"])
        cell_add_para(cell, "RATIONALE", bold=True, color=MD3["on_surface_variant"], size=9)
        for para in p25["rationale"]:
...
def build_phase5(doc, p5):
    ...
    for i in range(0, len(els), cols):
        ...
        for j in range(cols):
            ...
            if j < len(row_els):
                ...
                # cascade note
                cn = c.add_paragraph()
                cnr = cn.add_run(el.get("cascade_note", ""))
    ...
    # chain diagram
    if p5.get("chain_diagram_text"):
        doc.add_paragraph()
        add_sub_title(doc, "Cascading Impact · 根因链")
        cell = add_card(doc, bg_hex=MD3["surface_container"])
        p = cell.add_paragraph()
        r = p.add_run(p5["chain_diagram_text"])
        r.font.name = "Consolas"
        r.font.size = Pt(9.5)
        _run_color(r, MD3["on_surface"])
        if p5.get("chain_summary"):
            sp = doc.add_paragraph()
            sr = sp.add_run(p5["chain_summary"])
...
def build_phase6(doc, p6):
    ...
    # Strategies — use a side badge via leading emoji number + purple title
    for idx, s in enumerate(p6.get("strategies", []), start=1):
        ...
        _cell_margins(cell, top=180, bottom=180, left=220, right=220)
        tp = cell.add_paragraph()
        # Badge-style "S1" label
        bd = tp.add_run(f"  S{idx}  ")
        bd.bold = True
        bd.font.size = Pt(10)
        _run_color(bd, MD3["primary"])
        tr = tp.add_run(f"   Strategy {idx} · {s.get('title', '')}")
        tr.bold = True
        tr.font.size = Pt(13)
        _run_color(tr, fg)
        if s.get("tag"):
        ...
        if s.get("rationale"):
            ...
            rr = rp.add_run(f"Rationale: {s['rationale']}")
            rr.italic = True
            rr.font.size = Pt(10)
        ...
        for w in s.get("weeks", []):
            ...
            wr.bold = True
            wr.font.size = Pt(9)
            _run_color(wr, MD3["on_primary_container"] if idx == 1 else MD3["primary"])
            tr = wp.add_run("   " + w.get("text", ""))
            tr.font.size = Pt(10.5)

    # Metrics — each as a small card
    if p6.get("metrics"):
        doc.add_paragraph()
        ...
        for m in p6["metrics"]:
            ...
            tag = p.add_run(f"   ← {m.get('strategy_tag', '')}")
            tag.bold = True
...
def build_footer(doc, meta):
    ...
    doc.add_paragraph()
    p = doc.add_paragraph()
    r = p.add_run("Generated by Opportunity Progression Skill · AWS Sales Agent")
    r.italic = True
    r.font.size = Pt(9)
    ...
    if meta.get("report_date"):
        ...
        r2 = p2.add_run(
            ...
            + (f"  ·  Scorecard: {meta['scorecard_version']}"
               if meta.get("scorecard_version") else "")
        )
        r2.font.size = Pt(8)
...
def build_document(data: dict, out_path: Path) -> Path:
    ...
    build_phase25(doc, data.get("phase25"))
